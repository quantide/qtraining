...
Now sparklyr needs to use the spark binary and configuration of the cluster. This task is achieved setting the `SPARK_HOME` environment variable with the Spark path on the edge file system. Then the master option is `yarn-client` in this case. 
...
Actually it is not that useful reading a file from local, because of using a single hard drive is a bottleneck to access big data. So the typical use case is:

1. some one prepared a big dataset on HDFS
...
# or more explicitely
csv_file <- "hdfs://quickstart.cloudera:8020/user/hive/warehouse/sample_07/sample_07"
...
Poorly speaking, this piece of software perform the following operations:
