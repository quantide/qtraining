...
1. Look in the usbkey in folder `luiss-course/VirtualBox` and execute `VirtualBox-5.1.6-110634-Win.exe` 

2. open VirtualBox

3. on the menu click on `Import Appliance` and select `luiss-course/vb-qtraining-rserver.ova`

4. when the import process finish, double click on the machine to start it.

5. RStudio-server: open your browser at http://localhost:8080 (the default port is `8787`)

6. ssh-server: on port `2222` (not the default `22`)
