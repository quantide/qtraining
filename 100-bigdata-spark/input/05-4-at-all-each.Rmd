...
1. `summarise()`
2. `summarise_each()` -- Notice that `summarise_each()` will be deprecated in favour of `summarise_at()` and `summarise_all()`
3. `summarise_at()`
...
`summarise_each` will be deprecated in favour of `summarise_at()`:
...
As `summarise_each` will be deprecated, you may use `summarise_at()`:
...
A modern alternative to `summarise_each()` is  `summarise_at()`.
...
1. `mutate()`
2. `mutate_each()` -- Notice that `mutate_each` will soon be deprecated in favour of `mutate_at` and `mutate_all`
3. `mutate_at()`
...
`mutate_each` will be deprecated in favour of `mutate_at()`:
...
Alternatively, `mutate_at()` may be used:
