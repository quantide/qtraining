...
In this chapter you'll work mainly with the package `tidyr` which is part of the `tidyverse` core. 

As a data scientist you collect data from different sources and, very often you will have very little control over the original structure of this data.

Such a variety in data structure implies a long time spent on manipulating your data prior any analysis or visualization step.

On the other hand, you may think about a well defined and standard way of storing your data so that data are ready to be pushed into any next step. 

This _standard_ format has been defined as __tidy__.  Tidy data will allow you to work with all the tools given by `tidyverse`.
...
Tidy data makes it easy for an analyst or a computer to extract needed variables because it provides a standard way of structuring a dataset. Tidy data is particularly well suited for vectorized programming languages like `R`, because the layout ensures that values of different variables from the same observation are always paired. As all the tidyverse tools are based on tidy data, once your data is tidy you can easily apply any of these tools.


# Tidying Messy Datasets

Real datasets can, and often do, violate the three precepts of tidy data in almost every imaginable way. These are the five most common problems with messy datasets:
...
You can tidy this data by splitting the original data frame into two data frames: `df1` and `df2`.
...
This is often the case when you are loading data from different data sources.
...
then combine by rows the two data frames
...
In this example we work with data on measurements of the time people spend on their phones. The time spent on the phone is measured at two locations (work and home), at two times. Each person has been randomly assigned to either treatment or control.
...
To tidy this data, before using `gather()` it is necessary to turn columns `work.T1`, `home.T1`, `work.T2` and `home.T2` into a key-value pair composed by key and time. 
...
Supposing you have the following packages installed, load them into `R`:
...
In the following code chunks you will find also some `dplyr` functions. To better understand them, see the previous chapters of the course, in particular the `dplyr` parts about `mutate()` and `summarise()`.  
