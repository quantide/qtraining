...
Among packages that deal with dates and times in R, `lubridate` provides a set of intuitive and coherent functions. Intuitive since each function usually performs a single task described by its name; coherent since `lubridate` is part of tidyverse thus well integrated with the rest of it.

Supposing `lubridate` is currently installed, it must be loaded in order to use its functions (it is not part of the core of tidyverse, thus it is not loaded together with the rest of tidyverse).
...
`lubridate` provides a single function for each of these cases: `dmy()`, `mdy()` and `ymd()`. These functions parse dates according to the order provided, where `y` is the _year_, `m` is the _month_ and `d` is the _day_.
...
According to the help, `ymd()`-style _functions recognize arbitrary non-digit separators as well as no separator. As long as the order of formats is correct, these functions will parse dates correctly even when the input vectors contain differently formatted dates_.
...
At this point, the new `date` column is a string and it must be parsed to be interpreted as a date by `R`.
...
The `lubridate` package provides a set of functions to extract single components of a date: `month()`, `day()` and `year()`. Using just `lubridate` and `dplyr` the components of a date can be added as new variables to a data frame.
...
The last function showed here is `wday()`: it extracts the week day of a date, returning a number from Sunday (1) to Saturday (7).
...
Of course, also the new variable can be used as a grouping variable for box plots.
...
# Time Parsing
Package `lubridate` allows you to deal with times, too. Times have no meaning without dates, so you will parse the whole date-time string.

When data has date and time in two different variables, you can merge it in a single variable with a bit of `tidyr`. The following example requires data frame `diabetes` from `qdata` package.
...
# Time Zones
Date and times may vary with reference to timezones. When you define a `POSIXct` class object with parsing function seen above, you can refer to a specified timezone.
...
As an example, you can substract an hour to October 30, 2016 at 2.00 AM. The `hours()` function returns 1.00 AM, since it just subtracts an hour; the `dhours()` function returns 2.00 AM again, because in this night DST ends and clocks are set back, as if to repeat one hour.
