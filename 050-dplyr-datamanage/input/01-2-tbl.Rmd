...
or you can load it explicitely together with `dplyr` that will be useful throughout this chapter:
...
`dplyr`, as well as other packages that you will work with in this course, can work with data frames as is but it is worthwhile to convert them to a `tbl_df`, especially if frames have large dimensions. `tbl_df` is a wrapper around a data frame that will not accidentally print a lot of data to the screen, indeed tbl objects only print a few rows and all the columns that fit in one screen, describing the rest of it as text.
