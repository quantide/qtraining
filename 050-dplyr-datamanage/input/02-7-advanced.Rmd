...
You may also use function `summarise_at()`:
...
Alternatively to function `summarise_each`, you can use `summarise_at`:
...
Again, naming output variables with a different notation has to be achieved with a separate instruction:
...
Last, suppose you want to apply many functions to all of you variables, for example you want to calculate the mean and the standard deviation of the numeric variables in your dataset, a quick way for doing it is to use the function `summarise_all()`:
...
The default output variables names are `variable_function`. However, if you want to change them, you need to specify the new names with an additional command:
...
In addition you could use function `mutate_at()` as well, but its usage results again in a loss of clarity:
...
An alternative to `mutate_each` is `mutate_at`, that provides a compact notation:
...
Again, if you wish to assign a variable name different from the default one, you shall appropriately rename the __functions__ you call within `funs()`:
...
`mutate_all()` can be used also in the case in which you want to apply many functions to many variables:
...
The default name of the new variables are given by `variable_function`. In order to name output variables with a different notation, a separate instruction is needed:
...
You have two alternatives to functions `summarise()` and `mutate()`: `summarise_each()` and `summarise_at()`, `mutate_each()` and `mutate_at()` respectively.
...
Notice that, in a future release, the functions `summarise_each()` and `mutate_each()` will be depreacted in favour of the families of functions 1) `summarise_all()` and `mutate_all()` that apply a function to all columns and 2) `summarise_at()` and `mutate_at()` that operate on a subset of columns.
