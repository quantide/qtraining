...
```{missing, include=TRUE, purl=TRUE}
...
```{missing2, include=TRUE, purl=TRUE}
y <- c(1, 2, 3, "")
```
...
```{na_if, include=TRUE, purl=TRUE}
na_if(y, "")
```

and on the data frame `df`:

```{na_if2, include=TRUE, purl=TRUE}
...
```{drop missing, include=TRUE, purl=TRUE}
df_new %>% drop_na()
```
...
```{ignore missing, include=TRUE, purl=TRUE}
df_new %>% 
