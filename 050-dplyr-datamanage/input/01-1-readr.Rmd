...
In this chapter you'll learn how to import flat files in R. You will do this with the use of `readr` package which is part of the `tidyverse` core. As already learned, this means that if you load the package `tidyverse`, `readr` will be automatically loaded:

```{r readr}
require(tidyverse)
```

Alternatively, one may explictely load the package `readr`, together with `dplyr` and `tibble` that are the other packages used in this chapter:
...
If you open the tennis.txt file you notice that 1) variables are delimited by many columns of space and 2) variable names are at the top of the file. In this case the function `read_r_table` comes in handy.  `read_r_table` is designed to read the type of text data where each column is separated by one (or more) columns of space. With regards to 2) you just have to keep it in mind so that you indicate it in the function `read_r_table`.
...
Notice that `col_names = TRUE` indicates that the variables names are at the begininning of the file. 
...
A specific section is dedicated to strings and dates as they are not as straightforward to manipulate and specific packages are avaiable in tidyverse.
