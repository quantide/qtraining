...
In this chapter you'll learn how to import flat files in R. We will do this with the use of `readr` package which is part of the `tidyverse` core. As already learned, this means that if you load the package `tidyverse`, `readr` will be automatically loaded:

```{r readr}
require(tidyverse)
```

Alternatively, one may load explictely the package `readr`, together with the package `dplyr` and `tibble` that will be useful throughout this chapter:
...
Suppose you want to import data from tennis.txt (available in the data folder). If you open the file itself you notice that 1) variables are separated by many columns of space and 2) variable names are at the top of the file. The function `read_r_table` comes in handy as it is designed to read the type of text data where each column is separate by one (or more) columns of space. In order to import the text file, first of all you need to give the coordinates of the folder in which the text file is stored. First of all, let us view what is the directory where we are currently working:

```{r getwd, eval = FALSE}
getwd()
```

and secondly let us use the `set.wd()` function that allows you to set the working directory on the folder containing the data:
...
This method does not always work, especially when the file is extremely large. There are two basic problems:
...
* numbers: double (decimals) or integer, i.e. for numeric variables
* factors: categorical variables that have a known set of possible values
* strings: textual data
* date and times

A specific section is dedicated to stringsm factors and dates as they are not as straightforward to manipulate and specific packages are avaiable in tidyverse.
