...
# chefmozcuisine
chefmozcuisine <- read_delim("/home/emanuela/dev/qtraining/050-dplyr-datamanage/exercises/data/chefmozcuisine.csv", delim = ",", col_names = T)

# userprofile
userprofile <- read_delim("/home/emanuela/dev/qtraining/050-dplyr-datamanage/exercises/data/userprofile.csv", delim = ",", col_names = T)

# rating_final
rating_final <- read_delim("/home/emanuela/dev/qtraining/050-dplyr-datamanage/exercises/data/rating_final.csv", delim = ",", col_names = T)
```
...
# notice that placeID is an integer value. However, it is the ID hence you will calculate no statistics on it. You may as well force it to a character vector.
...
rating_all %>% 
  ...
  summarise_at(vars(rating_mean = rating, food_rating_mean = food_rating, 
                    service_rating_mean = service_rating), funs(mean)) %>%
...
rating_all %>% 
  group_by(activity) %>%
  summarise_at(vars(rating, food_rating, service_rating), funs(n(), mean, sd))
