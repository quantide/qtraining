...
## Data import: set working directory

In this section you will work with data are contained in the data folder. Hence you should set your working directory in the _data_ folder, using `setwd()` function, like in this example:

```{r setwd_fake, eval=FALSE}
setwd("~/Documents/datamanage/exercises/data)
```
...
aire <- read_delim("aire_milano_strings.txt", delim = "\t")
```
...
```{r, echo = show_solution, eval = FALSE}
...
aire_clean <- aire_clean %>% 
  mutate(MotivoIscrizioneEstero = str_replace(MotivoIscrizioneEstero, 
                                              c("^acquisto cittadinanza"), "per acquisto cittadinanza")) 
