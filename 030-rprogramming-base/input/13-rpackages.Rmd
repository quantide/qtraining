...
A very useful text is [R packages book](http://r-pkgs.had.co.nz/) by Hadley Wickham. The book is available online at the web address http://r-pkgs.had.co.nz/, and it is a very wide guide to packages creation. 
...
![Package creation via RStudio interface](images/package_creation.png)

In both cases, you will get a folder looking like this one:

![Package folder after creation](images/package_folder_beginning.png)
...
![Object documentation](images/documentation.png)
...
The tests/ folder may contain automatic tests for your package. The presence of automated tests can help you during the package development, and assure that every change you make does not affect the rest of your code in an unpredicted way.

A convenient way to develop automated test is through the package `testhat`. 
You can discover more about its usage in the Testing chapter, so here we will se just a quick example.

To set up our package to use testthat, we can run:
...
The procedure to add a new set of test is then:

1. Add a new .R file into the `tests/testthat` directory. The file name should start with the word _test_

2. Write the test 

3. Test our package with `Ctrl/Cmd + Shift + T` or `devtools::test()`.

Let's run an example for our `dice()` function. 

Suppose we want to be sure that the `rolls` argument works properly, determining the length of the output vector.

We can then add a `test_rolls.R` file in the `tests/testthat` directory, and write the test:
...
If we then test the package with `Ctrl/Cmd + Shift + T` or `devtools::test()`, we get as output:
...
If one of the test is not satisfied, we will receive a message error. 

If for instance we modify the second test as follow:
...
and we run the test, then we will get the message:
    ...
    Loading required package: testthat
    Testing game
    Dice function output: .1.

    Failed -------------------------------------------------------------------------
    1. Failure: Dice rolls sets up the number of rolls (@test_rolls.R#7) -----------
    length(dice(rolls = 10)) not equal to 60.
    1/1 mismatches
    [1] 10 - 60 == -50
...
Sometimes we want to make available one or more datasets along with our code. The right location for them is the data/ folder.

The easiest way to add data to our /data folder is to use the `devtools::use_data()` function.
...
Using `devtools::use_data()` we assure that all the data requirements will be respected: in fact, data in the data/ folder should be a .RData file created by save() containing a single object (with the same name as the file). 

All the objects in the data/ folder are exported, and should be documented.

To document a dataset you should add a roxygen block to your .R file, documenting the name of the dataset. 

Two extra @tagName are available for datasets:

* `@format` describes the dataset format

* `@source` describes the dataset source, often a `\url`

For instance, this is the documentation for the `diamond` dataset, contained in the `gplot2` package:
...
If you want to use data in your package without making them available to external user, you should save them in `R/sysdata.rda`
...
The inst/ folder contains all the extra material that you want to distribute together with your package.

Usually it may contain:

* Plain text files describing authorship, copyright and rules for a citation of a package, if they are particularly complex. Usually they are saved in subdirectory called inst/AUTHOR, inst/COPYRIGHT  and inst/CITATION

* External data for examples and vignettes, usually saved in the subdirectory inst/extdata

* Supplementary scripts in other language. The convention is to put scripts of this nature into a subdirectory of inst/, inst/python, inst/perl, inst/ruby etc. If these scripts are essential to your package, make sure you also add the appropriate programming language to the SystemRequirements field in the DESCRIPTION. (This field is for human reading so don’t worry about exactly how you specify it.)

You are free to put anything you like in inst/ with one caution: because inst/ is copied into the top-level directory, you should never use a subdirectory with the same name as an existing directory. This means that you should avoid inst/build, inst/data, inst/demo, inst/exec, inst/help, inst/html, inst/inst, inst/libs, inst/Meta, inst/man, inst/po, inst/R, inst/src, inst/tests, inst/tools and inst/vignettes.


### Other 

A directory that you may find sometimes useful is `src/`. This is the place where you can put compiled code (like C or C++) that can powerfully complement your R code. 

There are other directories that are rarely used: `demo/` for package demos, `exec/` for executable scripts, `po/` for messages translations, `tools/` for auxiliary files needed during configuration, or for sources that need to generate scripts.
