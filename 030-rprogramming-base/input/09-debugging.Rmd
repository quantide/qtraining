...
The `debugonce()` allows you to enter the local function environment the first time you run the function and go through the instructions one by one to see how it works. If you are using R Studio, bote that the environment is now the environment of the function you are debugging, and a nw script window contains the code of the function you are debugging.

The error is that we are not updating the content of `primes` vector, so let's change it:
...
Let us now see a slightly different way to open the function environment. If you place `browser()` in the code you want to debug, the debugger will open when it encounters it. So for example, in the following code, the debugger will open just after the vector `primes` is populating in the for loop:
...
`debugonce()` allows you to enter the function once, so that you find the error and fix it. Notice that there exist the function `debug()` as well. It allows you to enter the local environment of the function every time you run the function. This means, however, that you need to exit the debugger once you have finished debugging the function.
...
This will allow you to see where the function stopped. Now, you may want to go closer into the function, you may want to go and see inside the function scope. Hence you may run the function `options(error=browser)`. This means that when R finds an error, it will stop right at the moment the error occured and let you look inside the function environment.
...
So the error is that our function is designed for only working with vectors of length 1. We may try to correct the error, for example by vectorising the function.
...
<!--
C'è esempio di traceback migliore?
-->

Once you are done, you should turn off the error browser:
...
<!--
Aggiungere debugging da RStudio ?
-->
