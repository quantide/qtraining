...
We mentioned faceting in "How to make an histogram" chapter. In the following paragraphs we will examine in depth how to handle with the most common questions on faceting customization.
...
pl <- ggplot(data=bands, mapping=aes(x=ink_pct, fill=press_type)) +
  geom_histogram() 
```
...
<!--
si può togliere il grafico originale?
-->
