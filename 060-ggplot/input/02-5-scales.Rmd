...
But firstly, let us introduce some theoretical concepts about scales.

## Some theory about scales
...
For example, let us suppose that you want to increase the luminance of the points colours in the plot. Colour aesthetic mapping is controlled by `scale_color_discrete()` and the argument of `scale_color_discrete()` that control the luminance is `l`. `l` can assume values from 0 to 100 and its default value is 65. We want to increase the luminance and set it to 90 in this way:
...
There are more types of scales. They can be grouped into:
...
In the following paragraphs we will answer to the most frequent questions about mapped aesthetics scales customization for: colours, linetype, shape and size. The use of continuous position scales will be discussed in "Axes customization".
