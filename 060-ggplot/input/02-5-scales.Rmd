...
The appearance of mapped aesthetics is changed by scales.
...
In this chapter we discuss how changing the appearance of mapped aesthetics by exploring scales functionalities and customization.

<!--
We will talk about axes and legends in "More on Axis" and "More on scales" chapters, respectively.  
-->
...
In the following paragraph we will discuss the most frequent questions about mapped aesthetics customization, like colours, shape, linetype and size.
...
We will specify the colors for a continuous variable using various gradient scales. 
...
You can also using palettes produced by another package:
```{r}
ggplot(data = bands, mapping = aes(x = humidity, y = viscosity, colour = solvent_pct)) +
  geom_point() + 
...
Both aestetics can be mapped only to discrete variables.
...
Shape and linetype can be customized to emphasize the differences between the groups.

The allowed shapes symbols are:
...
If you want to customize shape scale you have to set `values` argument of `scale_shape_manual_function()`:

```{r}
ggplot(data=bands, mapping=aes(x=humidity, y=viscosity, shape=press_type)) +
  geom_point() +
  scale_shape_manual(values = c(9,10,11,12))
```
...
You can also using radiuos to control the size of plotting symbols by setting `scale_radius()` function:
