...
For example, let us suppose that you want to reduce the luminance of the points colours in the plot. Colour aesthetic mapping is controlled by `scale_color_discrete()` and the argument of `scale_color_discrete()` that control the luminance is `l`. `l` can assume values from 0 to 100 and its default value is 65. We want to increase the luminance and set it to 40 in this way:
...
In the following paragraphs we will answer to the most frequently asked questions about mapped aesthetics scales customization for: colours, linetype, shape and size. The use anf the features of continuous position scales will be discussed in "Axes customization" chapter.
...
Line type values can be pass as numbers or strings.
