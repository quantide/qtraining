...
data. The amount of smoothing depends on the kernel bandwidth: the larger the bandwidth, the more smoothing there is. The bandwidth can be set with the `adjust` parameter, which has a default value of 1.
...
`ggplot2` function that draws a frequency polygon is `geom_freqpoly()`:
