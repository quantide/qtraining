...
Scales, `theme()` and `guides()` functions are the tools of `ggplot2` used to customize the appearance of the legend.
...
Suppose we want to analyze the relationship between humidity, viscosity, ink percentage and band typen `bands` dataset: 
...
Looking at the plot we see that legends are represented in different lines.

If you want to set legends in the same line set `legend.box` argument of `theme()` function as `"horizontal"`:

```{r, warning=FALSE}
pl_2 + theme(legend.position="bottom", legend.box = "horizontal")
```

The legend can also be placed inside the graphing area by specifying a coordinate position. In this case, the argument `legend.position` has to be set to a numeric vector c(x,y), whose values should be between 0 and 1. In particualar, the coordinate space starts at (0, 0) in the bottom left and goes to (1, 1) in the top right.
