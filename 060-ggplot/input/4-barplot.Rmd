...
This chapter presents how to build bar plots and introduces other basic concepts of `ggplot2` graphics: `stat`.
...
# Setting and mapping variables to line plots
...
As we see in the previous plots, `cylinder_size` is a factor variable with three levels: `CATALOG`, `SPIEGEL` and `TABLOID` but it has also missing values (`NA`).   
To visualize also missing values (`NA`) as `cylinder_size` level in plots legend, we have to recode its levels, adding also `NA` as level.  
