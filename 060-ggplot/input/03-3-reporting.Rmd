...
If you are taking notes or you are writing a manual for other R users, you may want to show the code along with the output and some other times you may want to show just the code. Some other times, for examples if you are just creating a report, you may want to hide the code and only show the output. These sorts of actions can be changed for all code chunks by adding some options to each code chunk.

* __Hide code__: for hiding code, you just need to add to your code chunk ``{r, echo = FALSE}``

* __Hide output__: if you want to show the outuput but not the code, add the option ``{r, echo = FALSE}``

* __Hide messages__: messages regarding for example packages loading can be hidden by using the option ``{r, message = FALSE}``

* __Hide warnings__: the printing of warnings can be can be hidden by using the option ``{r, warning = FALSE}``

* __Hide code and results__: if you want to hide both code and results, you may use ``{r, include = FALSE}``
