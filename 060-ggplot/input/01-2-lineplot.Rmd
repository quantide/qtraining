...
You can change the appearance of the line setting `geom_line()` aestethic arguments. For example, you can set dark blue color for line and chenge the size in this way:
...
As you can see from the plot no lines are drawn and a warning message is returned. Probably, this is one of the most frequently asked about `geom_line()`. This appens because `geom_line()` tries to connect data points that belong to same group and the combinations of different levels of factor variables belong to different group. 
