...
`ggplot2` allows one to handle with the appearance of the axes by using two important elements of its grammar: 
...
In the following paragraphs we will see how to handle with the most common questions on axes customization. 
...
Note: There is empty space instead of  not displayed `press_type` levels ("ALBERT70", "MOTTER94"). Maybe it is a bug of 2.1.0 version of `ggplot2`, used to realize this manual.
...
Suppose you arew interested in the relationship between ink and solvent percentage in `bands` dataset:
...
You can change as you want to resize the scales:
...
Let us consider the logarithmic transformation examples described in "Axis scale transformations" paragraph.
...
Talking about the plot in which scales have been transformed by natutal log and $log_{2}$:
...
`trans_breaks()` function of `scales` package define the breaks sccording to the transformation applied.
...
We use a subset of the economics data, ranging from mid-1992 to mid-1993. If breaks aren’t specified, they will be automatically selected, as it appens with numerical variables:
...
The format specified is to be put in a string that is passed to `date_format()`, and the format specifiers will be replaced with the appropriate values. For example, if you use "%B %d, %Y", it will result in labels like "June 01,
1992".
...
<!--
inserire using relative times on axis?
-->