...
# La directory di lavoro va definita all'inizio in questo modo per non doverla ripetere in
# ogni chunk! (con echo=F si eseguono i comandi del chunk senza mostrarli)
opts_knit$set(root.dir = "/home/enrico/Dropbox/quantide/int/corsi/corsiR/21.rModels/v01/data/allData")
```
...
The following trellis graph shows if some relations exist between `Status` and `Age`:
```{r,fig.cap="Box-plot of Age within levels of Status"}
  bwplot(Age~Status,data=titanic,main="Box-plot of Age within levels of Status")
```
...
![Example of simple feed-forward Neural Network](./figure/nnet.png)
