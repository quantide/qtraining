...
The most important continuous distribution is the normal curve. The name normal curve comes from the convinction (not always correct) that a great number of biological or physical phenomena are usually distributed according the Gaussian curve.  
...
Given $n$ independent random variables $Z_1,\;Z_2,\;  \dots,\; Z_n$, normally distributed with $\mu = 0$ and $\sigma = 1$, the sum of squares of $Z_i$ is called a Chi-square random variable; in symbols: $\chi_\nu^2$, where $\nu = n$.
The density function of the $\chi^2$ depends only from the parameter $\nu$, the so called number of _degrees of freedom_ ($df$).
The Chi-square distribution is defined between $0$ and $+\infty$, and when the number of degrees of freedom increases, it tends to have a shape similar to that of the Normal distribution.
...
<!--- The Student’s $t$-distribution takes into consideration the relations among the mean and the variance estimations, in small sample sizes, when the sample variance is used.  
--->
...
where $s^2$ is the so called sample variance, is distributed as a Student's $t$ with $n - 1$ degrees of freedom.  
The corresponding curve is defined between $-\infty$ and $+\infty$, is symmetric, little lower than the normal around the median, and with higher frequencies in the extremes.

In statistical tests, the choice between the use of the Normal distribution or the Student’s $t$-distribution in comparisons between means depends on the fact that
the $\sigma^2$ variance is known or not. If $\sigma^2$ is unlknown, the $s^2$ sample variance and the $t$ distribution must be used.
...
Another interesting distribution, on which a big part of the parametric statistics is founded, is the $F$-distribution.
...
The $F$ distribution ca be seen as a generalization of the $t$ distribution.
The $t$ and $\chi^2$ distribution uses only one parameter, the $F$ has two parameters: the number of degrees of freedom both of the numerator and of the denominator.
The square of a Student's $t$ random variable with $n$ degrees of freedom is equal to an $F$ distribution with $1$ and $n$ degrees of freedom.
