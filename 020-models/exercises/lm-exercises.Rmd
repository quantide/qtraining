---
title: "Exercises of Models Course"
author: "Quantide Srl"
date: "July 6, 2016"
...
A society that produces ballpoint pens uses a multi-head machine for the production of caps for pens. The productor wants to determine if there are differences on the means of the different heads.
...
```{r desc_stats_an1, echo=show_solution, eval=show_solution}
...
```{r data_analysis_an1, echo=show_solution, eval=show_solution}
...
```{r data_analysis_an2, echo=show_solution, eval=show_solution}
op <- par(mfrow = c(2,2))
plot(fm)
par(op)
...
A chemist conducts an experiment to evaluate the efficacy of a solvent to dissolve stains of nail
varnish from fabrics. He uses two types of solvent and three types of varnish. The experiment consists of immersing 5 stained fabrics by a certain type of varnish into a bowl with a solvent, measuring the time
(in minutes) necessary to dissolve the stain.
...
```{r desc_stats_an2, echo=show_solution, eval=show_solution}
# Descriptive Statistics of Time for each level of Solvent 
summary_stat <- varnish %>% group_by(Solvent) %>% 
  summarise(n=n(),
  min = min(Time), 
...
# BW Plot of Time for each level of Solvent
varnish <- varnish %>% mutate(Solvent=factor(Solvent))
ggp <- ggplot(data=varnish, mapping = aes(x=Solvent, y=Time, fill=Solvent)) +
  geom_boxplot(show.legend = FALSE) 
print(ggp)



# Descriptive Statistics of Time for each level of Varnish 
summary_stat <- varnish %>% group_by(Varnish) %>% 
  summarise(n=n(),
  min = min(Time), 
  first_qu = quantile(Time, 0.25),
  median = median(Time),
...
# BW Plot of Time for each level of Solvent
varnish <- varnish %>% mutate(Varnish=factor(Varnish))
ggp <- ggplot(data=varnish, mapping = aes(x=Varnish, y=Time, fill=Varnish)) +
  geom_boxplot(show.legend = FALSE) 
print(ggp)

# BW plot for each combination of Solvent and Varnish factor levels
ggp <- ggplot(data = varnish, mapping = aes(x=Varnish, y=Time, fill=Solvent)) + 
  geom_boxplot()
...
```{r data_analysis_an21, echo=show_solution, eval=show_solution}
fm <- aov(Time ~ Varnish * Solvent, data = varnish)
summary(fm)

# Actually, only main effects Solvent seems to influence the time necessary to dissolve the stain.  Interaction seem not significant: we can start to drop it.
```

3. Let us improve the model, dropping non significant terms (one by one).

```{r data_analysis_an22, echo=show_solution, eval=show_solution}
# Drop the interaction from model formulation Varnish:Solvent
fm1 <- update(fm, . ~ . -Varnish:Solvent)
summary(fm1)
# Drop Varnish effect from model formulation
fm2 <- update(fm1, . ~ . -Varnish)
summary(fm2)
```

4. Let us check if the removed effect/s are not significant (models comparison).

```{r data_analysis_an23, echo=show_solution, eval=show_solution}
anova(fm, fm2)
# The removed effects are not significant.
```

5. Let us check (final) models residuals.

```{r data_analysis_an24, echo=show_solution, eval=show_solution}
op <- par(mfrow = c(2,2))
plot(fm2)
par(op)
# The residual plots confirm that normality and homoscedasticity assumptions are met, and no outliers appear.
...
# Correlation
paint %>% summarise(cor=cor(Stirrate, Impurity))
...
# The relation that "links" the revolution rate seems to be well described by a simple linear regression, at least for the considered interval of Stirrate. It appears that if we increase Stirrate by one unit, impurities increase by 0.46 on average, and this growth is significant. The p-value for the significativity of the coefficient of Stirrate is extremely low. This fact confirms the "non random"" existing relation between the two variables. The p-value of the intercept estimate is 0.817. This would indicate an intercept potentially equal to 0. Keeping the intercept value equal to the estimate, with value of Stirrate=0, the mean number of imputities would be negative: the results of the regression couldn’t be "extended" too outside experimental field. R^2 value is equal to 0.9338; this fact confirms that the model explais well the observed data.
```
...
```{r mod_improve_lm21, echo=show_solution, eval=show_solution}
# Improve the model 
...
```{r mod_improve_lm22, echo=show_solution, eval=show_solution}
...
```{r desc_stats_lm3, echo=show_solution, eval=show_solution}
ggpairs(data = knock)
...
```{r mod_improve_lm31, echo=show_solution, eval=show_solution}
...
```{r mod_improve_lm32, echo=show_solution, eval=show_solution}
anova(fm1, fm3)
# There is a non-significant difference between the two models. fm3 seems the better model.
```
...
```{r data_analysis_lm41, echo=show_solution, eval=show_solution}
# Fit the linear model
...
```{r data_analysis_lm42, echo=show_solution, eval=show_solution}
# Remove RelHum
...
```{r data_analysis_lm43, echo=show_solution, eval=show_solution}
...
```{r data_analysis_lm44, echo=show_solution, eval=show_solution}
# Models Comparison 
anova(fm1, fm11)
```
...
```{r data_import_lm5, echo=show_solution, eval=show_solution}
data(brainbod)
head(brainbod)
```
...
```{r desc_stats_lm51, echo=show_solution, eval=show_solution, message=FALSE}
# Body
...
2. Let us show a scatterplot of the two variable. Let us estimate a linear regression model and check the model residuals. 
The linear model between the two variables is adequate? Why?

```{r data_analysis_lm51, echo=show_solution, eval=show_solution, warning=FALSE}
# Scatterplot
pl3 <- ggplot(data = brainbod, mapping = aes(y=Brain, x=Body)) +
  geom_point(col="blue") + geom_smooth(method = "lm", colour= "red", se=FALSE)
...
3. How could the model be improved? What variable transformation can be applied? Generate BW plot, histograms and scatterplot of the transformed variables.

```{r desc_stats_lm52, echo=show_solution, eval=show_solution, message=FALSE}
# Body
...
5. Let us estimate a linear regression model with the transformed variables and check the model residuals.
```{r data_analysis_lm52, echo=show_solution, eval=show_solution, warning=FALSE}
# Fit the linear model of the tarnsformed variables
...
```{r desc_stats_glm1, echo=show_solution, eval=show_solution}
# Descriptive Statistics
summary(admission)
admission %>% summarise_each(funs="sd")

# Graphs of admit and GRE by rank
...
# Graphs of admit and GPA by rank
...
```{r data_analysis_glm11, echo=show_solution, eval=show_solution}
...
```{r data_analysis_glm12, echo=show_solution, eval=show_solution}
op <- par(mfrow = c(2,2))
plot(fm1)
par(op)
...
```{r predictions_glm1, echo=show_solution, eval=show_solution}
admission$prob <- predict(fm1, type = "response")
admission <- admission %>% arrange(prob)

# Prediction of admit vs gre by rank 
ggp <- ggplot(data = admission, mapping = aes(x = gre, y = admit)) +
  geom_point(colour="blue") + 
  geom_line(mapping = aes(x = gre, y = prob), colour="red") +
  facet_wrap(facets = ~rank)
print(ggp)

# Prediction of admit vs gpa by rank
ggp <- ggplot(data = admission, mapping = aes(x = gpa, y = admit)) +
  geom_point(colour="blue") + 
  geom_line(mapping = aes(x = gpa, y = prob), colour="red") +
  facet_wrap(facets = ~rank)
print(ggp)
```
...
# previsioni con intervalli (interessante ma + articolato)
...
```{r desc_stats_glm2, echo=show_solution, eval=show_solution, warning=FALSE}
# Descriptive Statistics
summary(awards)
...
```{r data_analysis_glm21, echo=show_solution, eval=show_solution}
...
```{r data_analysis_glm22, echo=show_solution, eval=show_solution}
## update m1 model dropping prog
fm2 <- update(fm1, . ~ . - prog)
summary(fm2)
## test model differences with chi square test
anova(fm2, fm1, test="Chisq")
# The best model is the complete model (first)
```

4. Let us compure predictions of the best fitted model.
```{r predictions_glm2, echo=show_solution, eval=show_solution}
