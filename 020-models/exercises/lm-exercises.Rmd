...
A chemists conducts an experiment to evaluate the efficacy of a solvent to dissolve stains of nail
...
2. Let us evaluate also if there are differences in the types of solvent in respect of the types varnish that produced the stain (Considerer also the interaction in model formulation).

```{r data_analysis_an21, echo=show_solution}
...
```{r data_analysis_an22, echo=show_solution}
...
```{r data_analysis_an23, echo=show_solution}
anova(fm, fm2)
...
```{r data_analysis_an24, echo=show_solution}
op <- par(mfrow = c(2,2))
plot(fm2)
par(op)
# The residual plots confirm that normality and homoscedasticity assumptions are met, and no outliers appear.
...
```{r data_import_lm1, echo=show_solution}
data(paint)
head(paint)
```
...
```{r desc_stats_lm1, echo=show_solution}
# Descriptive Statistics
...
```{r reg_plot_lm1, echo=show_solution}
...
```{r data_analysis_lm11, echo=show_solution}
...
```{r data_analysis_lm12, echo=show_solution}
# Residuals analysis
op <- par(mfrow = c(2,2))
plot(fm)
par(op)
...
```{r data_import_lm2, echo=show_solution}
data(switcht)
head(switcht)
```
...
```{r desc_stats_lm2, echo=show_solution}
# Descriptive Statistics
...
```{r reg_plot_lm2, echo=show_solution}
...
```{r data_analysis_lm2, echo=show_solution}
# Fit the linear model
...
```{r mod_improve_lm21, echo=show_solution}
# Improve the model 
...
```{r mod_improve_lm22, echo=show_solution}
...
```{r data_import_lm3, echo=show_solution}
data(knock)
head(knock)
```
...
```{r desc_stats_lm3, echo=show_solution}
ggpairs(data = knock)
...
```{r data_analysis_lm3, echo=show_solution}
# Fit the linear model
...
```{r mod_improve_lm3, echo=show_solution}
...
```{r data_import_lm4, echo=show_solution}
data(mortality)
head(mortality)
```
...
```{r data_analysis_lm41, echo=show_solution}
# Fit the linear model
...
```{r data_analysis_lm42, echo=show_solution}
# Remove RelHum
...
```{r data_analysis_lm43, echo=show_solution}
...
```{r data_analysis_lm44, echo=show_solution}
# Models Comparison 
anova(fm1, fm11)
```
...
```{r data_import_lm5, echo=show_solution}
data(brainbod)
head(brainbod)
```
...
```{r desc_stats_lm51, echo=show_solution}
# Body
...
```{r data_analysis_lm51, echo=show_solution}
# Scatterplot
...
```{r desc_stats_lm52, echo=show_solution}
# Body
...
```{r data_analysis_lm52, echo=show_solution}
