...
The R package `XLConnect` permits to create a formatted spreadsheet usable as a dynamic report of the R analisys. 
...
`loadWorkbook` function creates an R object workbook in the path and with the name specified by  `filename` argument. It creates it ex-novo, as `create` argument is set as `TRUE`.  
...
Suppose we want to add a dataset to the sheet just created. We want to add `airquality` dataset available in `datasets` package:  
...
The function `setCellFormula` apply the formula specified by the argument `formula` to the rows specified by `row` of the column `col` of the sheet `sheet` of the R object.
...
To read an existing excel file, the syntax is the similar that for the creation:

<!--
exc2 <- loadWorkbook(fileXls, create=TRUE)
-->
...
`exc2` is a new XLC object created from and existing file excel (the old fileXls), and `readWorksheet` function reads data from `exc2` object. 
...
The new sheet *OzonePlot* is added to `exc2` object and the new named region *OzonePlot* is creating starting from *OzonePlot!$A$1* cell.
