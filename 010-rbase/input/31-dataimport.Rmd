...
---
```{r date-requirePackages, echo=FALSE, message=FALSE, warning=FALSE, results='hide'}
...
`loadWorkbook` function creates an R workbook object in the path and with the name specified by  `filename` argument. It creates it ex-novo, as `create` argument is set as `TRUE`.  
...
<!--
exc2 <- loadWorkbook(file_xls, create=TRUE)
-->
...
`exc2` is a new R workbook object created from and existing file excel (the old `file_xls`), and `readWorksheet` function reads data from `exc2` object. 
...
```{r rm_mtcars_sqlite, echo=FALSE, purl=FALSE, message=FALSE, results='hide'}
file.remove("mtcars_sqlite")
```
