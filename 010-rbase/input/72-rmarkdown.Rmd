...
You can also create code chunks that define functions in C++ using Rcpp Attributes. This is accomplished using the `engine = 'Rcpp'` chunk option. For example:
...
The Aritmetic mean is equal to $\dfrac{1}{n}\sum_{i=1}^{n} x_{i}$ or the summation of n number divided by n. 
...
<!--
## Summary
> In this chapter, we showed how to build a report with R Markdown. We started with the program installation and the most commonly used R Markdown syntax. Then we explained the features of the chunk and various options that can be applied to it. Finally we talk about the  ways to render an R Markdown document into it’s final output format.
