...
seqby
seqlength = seq(from = 1, to = 5, length = 13) 
seqlength
```
...
```{r, list create}
myList = list(vec = 1:7, mat = matrix(1:12, ncol = 3),
...
size = factor(c(2, 3, 1, 1, 1, 2, 3, 3), levels = c(1, 2, 3, 4),
  labels = c("small", "small", "large", "large"))
size
...
df = data.frame(name, height, gender, stringsAsFactors = F)
df
```
