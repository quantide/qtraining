...
<u>$\mu$</u> $=$ $E($<u>$y$</u>$) =$ $\mathbf{X}$ <u>$\beta$</u> $+$ <u>$\varepsilon$</u>

where:

 - $\mathbf{X}$ is a $n \text{ x } (p+1)$ matrix, with all ones in the first columns, 
 - <u>$\beta$</u> is the vector of $\beta_0, \beta_1, \dots, \beta_p$,
 - <u>$\varepsilon$</u> is a **random** or **noise** component.
...
The main assumption of linear model is that the model is linear in the parameters. Other assumptions concern the error terms <u>$\varepsilon$</u>are:
...
`fm` object is a list of elements containing information about regression results.   
...
Predict can be used for calculating prediction interval for the fitted model. Prediction interval for any new observation possibly within the regression support is given by:
...
Given  $\alpha=0.05$ as significance level, only some coefficients result as significant. Note that `PanB` expresses the difference between the intercept, aliased with _PanA_, and _PanB_; while `volume:panB` represent the difference in slope between _PanA_ (volume) and _PanB_. 
...
As a result, a generalized linear model with family Gamma and link identity can be used in place of a standard linear model.
...
Gamma model appears to be slightly better, as it has the lower AIC value.
