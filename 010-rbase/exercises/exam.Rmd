...
a. (2 pt) Generate a matrix, named `mat`, with 3 rows and 5 columns containig numbers from 1 to 15. Use `matrix()` function:

```{r ex2_a, eval=show_solution, echo=show_solution}
mat <- matrix(1:15, nrow = 3, ncol = 5)
```

b. (2 pt) Select 2-nd and 3-rd rows and 1-st and 3-rd columns of `mat`.
...
b. (2pt) Convert `continent` variable of `df` as a factor with levels: "Europe", "Asia" and "Africa". Use `factor()` function.
