...
Many of these packages are simply wrappers around one or multiple other parallelism packages forming a complex and sometimes
confusing web of packages. 
...
```{r parallel-024, label="fxx2", error=TRUE}
cluster <-  makeCluster(2)
...
And in fact `foreach` is a `R` library developed at __Revolution Analytics__: the major commercial supplier of R based solutions.

Package `foreach`  provides a new looping construct for executing R code repeatedly. 
```{r parallel-031, label="do"}
library(foreach)
foreach ( i=1:3) %do% log(i)
```

Function `foreach()` is similar to the standard function `lapply`, but doesn't require the evaluation of a function. 
Moreover, `foreach()` has some interesting functionality such as the combine argument that makes `foreach` a very versatile iterator. 
