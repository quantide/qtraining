...
Let us replay a similar analysis on the `life` dataset (see the section *Introduction and datasets used* for further information).

```{r 02a-life1}
str(life)
head(life)
ggpairs(data = life[,-1])
DLife <- dist(x = life[,-1], method = "euclidean")
(mdres <- cmdscale(d = DLife, k = 8, eig = TRUE))
```

```{r 02a-life2, fig.width=plot_with_legend_fig_width_short}
X_eig <- mdres$eig
P1 <- cumsum(abs(X_eig))/sum(abs(X_eig))
P2 <- cumsum(X_eig^2)/sum(X_eig^2)
mardia <- data.frame(Dimensions = 1:attr(DLife, "Size"), P1 = P1, P2 = P2)
p <- ggplot(mardia, aes(x = Dimensions))
p <- p + geom_point(aes(y = P1, color = "P1")) +
  geom_line(aes(y = P1, color = "P1"))
p <- p + geom_point(aes(y = P2, color = "P2")) +
  geom_line(aes(y = P2, color = "P2"))
p <- p + scale_x_continuous("Dimensions", breaks = 1:attr(D, "Size")) +
  scale_y_continuous("Value", breaks = seq(0, 1, by = .1)) +
  scale_colour_manual("Criterion", values = c("P1" = "blue", "P2" = "red")) +
  geom_hline(yintercept = .8, linetype = "dashed", color = "darkgray")
p
```
...
```{r 02a-life3}
eig <- mdres$eig
eig <- data.frame(Dimensions = 1:attr(DLife, "Size"), Eigenvalue = eig)
p <- ggplot(eig, aes(x = Dimensions, y = Eigenvalue)) + geom_point() +
  geom_line() + scale_x_continuous("Dimensions", breaks = 1:attr(DLife, "Size"))
p + geom_hline(yintercept = 0, linetype = "dashed")
```

```{r 02a-life4}
dt <- data.frame(mdres$points[,1:2])
gg <- ggplot(data = dt, aes(x = X1, y = X2)) + geom_point() +
  geom_text(label = life$country, hjust = 0.5, vjust = -0.5, size = 3)
gg
```

```{r require_reshape2, message=FALSE}
require(reshape2)
```

```{r 02a-life5}
dat.m <- melt(life[,-1], measure.vars = names(life[,-1]))
head(dat.m)
gg <- ggplot(dat.m, mapping = aes(x = variable, y = value)) + geom_boxplot()
gg
```
...
As a last example, we consider the `smacofIndDiff()` function, that allows one to
fit a so-called 3-way MDS, which analyzes many dissimilarity
matrices jointly, each one typically containing the judgments for a set of
objects provided by different subjects. The basic idea of 3-way MDS models is
to allow finding a different MDS solution for each subject.

The `smacofIndDiff()` function implements different 3-way MDS algorithms, among
which the `identity`, `INDSCAL` and `IDIOSCAL` models. In the identity approach,
the subject-specific configurations of the different subjects are constrained
to be the same. With INDSCAL, instead, the subject-specific solutions are
different, but they can be generated from one common solution by appropriate
weightings of the dimensions. The weights can be interpreted as the
"salience" of each dimension for each subject. If one drops also the
restriction of common dimensions for all individuals, a more general model
(IDIOSCAL) arises that allows for subject-specific rotations of the common
space.


### Example: Bread Ratings

Let's consider the `bread` dataset (see the section *Introduction and datasets used* for further information): we now look for an MDS representation of ratings of 10 breads on 11 different attributes carried out by 8 raters. Note that the bread samples are pairwise replications: each of the 5 different breads, which have a
different salt content, was presented twice for rating. The attributes are bread odor, yeast odor, off-flavor, color, moisture, dough, salt taste, sweet taste, yeast taste, other taste, and total. We first load the package `smacof`:

```{r, message=FALSE}
require(smacof)
```

The data are organized as a list of length eight. First we fit an
unconstrained solution (IDIOSCAL):

```{r 02a-idioscalbr}
res.idioscal <- smacofIndDiff(delta = bread, constraint = "idioscal")
res.idioscal
```

We can produce the plot of the common configuration space

```{r 02a-plotidioscalbr}
res.idioscal_res <- res.idioscal$gspace
p_idioscal <- qplot(x = res.idioscal_res[, 1], y = res.idioscal_res[, 2],
										geom = "point", xlab = "1st dimension",
										ylab = "2nd dimension",
										label = rownames(res.idioscal_res)) +
  geom_text(hjust = 0.5, vjust = -0.5, size = 3) +
  ggtitle("IDIOSCAL solution")
p_idioscal
```

The unconstrained solution reflects the bread replication pairs. The weights
attributed to the different dimensions by the different raters can be
accessed through the `cweights` argument returned by `smacofIndDiff()`:

```{r 02a-plotidioscalwbr}
w <- matrix(unlist(lapply(res.idioscal$cweights, diag)), nrow = length(bread),
            byrow = TRUE)
plot(w, type = "n", xlim = c(0, max(w)*1.1), ylim = c(0, max(w)*1.1),
     xlab = "1st dimension", ylab = "2nd dimension")
arrows(0, 0, w[, 1], w[, 2], length = .1, angle = 10)
text(w[, 1], w[, 2], labels = names(bread), pos = 4, cex = .8)
```

From the previous plot we see that the raters tend to judge the 11
attributes slightly differently along the two extracted dimensions. In
particular, rater 8 attributes the largest weight to the 1st dimension, while
rater 7 the assigns the largest weight to the 2nd dimension. Moreover, we
observe a clustering of the raters in three groups: {2, 7}, {1, 5} and
{3, 4, 6, 8}.


### Example: Life Data (Integration)

Let us consider life data again and correct an outlier in Trinidad's information (probably en error).

```{r 02a-life_integration}
life[22,"m25"] <- 43  # Outlier???
distm <- dist(x = life[,2:5])
distw <- dist(x = life[,6:9])
distt <- list(distm=distm,distw=distw)

res.life <- smacofIndDiff(delta = distt, constraint = "idioscal")

res.life_res <- res.life$gspace
p_idioscal <- qplot(x = res.life_res[, 1], y = res.life_res[, 2],
                    geom = "point", xlab = "1st dimension",
                    ylab = "2nd dimension",
                    label = life$country) +
  geom_text(hjust = 0.5, vjust = -0.5, size = 3) +
...
w <- matrix(unlist(lapply(res.life$cweights, diag)), nrow = length(distt),
            byrow = TRUE)
plot(w, type = "n", xlim = c(0, max(w)*1.1), ylim = c(0, max(w)*1.1),
     xlab = "1st dimension", ylab = "2nd dimension")
arrows(0, 0, w[, 1], w[, 2], length = .1, angle = 10)
text(w[, 1], w[, 2], labels = names(distt), pos = 4, cex = .8)
```
