...
![Schema SVM](./images/svm-schema.png)

When the classes are perfectly separated (__hard margin__), then SVM search for the
"line" that maximally separates the 2 classes data points.  
SVM then maximizes the margin $m$.  
Since the distance between the origin and the line $w^Tx = k$ is $\frac{k}{\left \| w \right \|}$, the margin $m=\frac{2}{\left \| w \right \|}$.
