...
As expected, the first two components explain almost all the distances between points in dataset.  
With the next computations we "project" the complete solution on the original space, to show 
"what" MDS actually did:

The following graph can be seen only at screen
...
As expected, the third component (`z`) in above graph shows a very little variation, with respect to the first two. 
Thus, if we want to analyze the 2D solution, we have simpy to set to 0 (or to exclude) the third component in mds solution, obtaining:
...
As we can see, 
And the observed vs. reproduced distances plot:
