...
are grown, the random forest (classificazion) predictor is
...
(Note: in case of regression prediction the random forest prediction, the procedure is
similar to the one shown above, but the prediction is made of:
$$\hat{f}_{rf}^B(x) = \frac{1}{B} \sum_{b=1}^B T_b(x; \Theta_b)$$ 
where the $T_b(x;\Theta_b)$ are the $B$ regression trees.)
