...
set.seed(1000)
train <- sample(nrow(cu.summary), 2000)
cu_train <- cu.summary[train,]
cu_test <-  cu.summary[-train,]

rf_fit <- randomForest(Mileage ~ Price + Country + Reliability + Type, data = cu.summary)
# Or, with train and test sets:
(rf_fit <- randomForest(x= cu_train[,c("Price", "Country", "Reliability", "Type")], y = cu_train$Mileage, 
                        xtest= default_test[,c("Price", "Country", "Reliability", "Type")], ytest = default_test$default)
plot(rf_fit)
```
