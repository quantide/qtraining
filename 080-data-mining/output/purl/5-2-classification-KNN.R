...
k_cv <- which.min(test_error)
knn_pred_cv <- knn3Train(train_X, test_X, train_Y, k = k_cv, prob = FALSE,
						 use.all = FALSE)
cm <- confusionMatrix(data = knn_pred_cv, reference = test_Y, positive = "Up")
ggp <- ggplot(data.frame(test_error), aes(x = 1:kmax, y = test_error)) +
	geom_line() +
  geom_point() +
  xlab("K (#neighbors)") + ylab("Test error") +
	ggtitle(paste0("KNN best K value = ", k_cv,
				   " (min error = ",
