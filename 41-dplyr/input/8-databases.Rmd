...
As these files might have different encodings you may need to define a simple function that returns the encoding of the file. Nothe that this function uses unix system commands and may not work under windows based computers.
...
You then create an empty SQLite database with a little trick that ensures the database is reoved in case it already exists:
