---
title: "Personalizzazione della Legenda"
...
Una legenda è una guida che illustra come risalire dalle proprietà estetiche ai dati originali.
`ggplot2` genera automaticamente la legenda in base alle estetiche mappate.

Vediamo un esempio, considerando il dataset `bands`. Supponiamo di essere interessati alla relazione tra umidità e viscosità in base alla presenza o meno di banda e costruiamo quattro box plot per analizzare le distribuzioni: 
...
Come possiamo vedere dal grafico, la legenda viene automaticamente costruita mostrando i livelli della variabile `press_type`. In questo caso, la variabile `press_type` è mappata sull'estetica `fill`. Quando una variabile viene mappata su un'estetica, viene automaticamente impostata una scala. La scala di default, in questo caso, è `scale_fill_discrete()`, ma ci sono anche altre scale in accordo alle estetiche mappate, come `colour` (per le linee e i punti) o `shape` (per i punti).

Le funzioni scala, `theme()` e `guides()` sono gli strumenti che `ggplot2` utilizza per personalizzare l'aspetto estetico dei componeneti della legenda.
...
Nei paragrafi seguenti, vedremo come risolvere i problemi più comuni in merito alla personalizzazione della legenda.
...
A volte la legenda è ridondante, o è già presente in un'altro grafico che sarà mosrato con quello corrente. In questi casi, può essere utilie rimuovere la legenda dal garfico. 

La legenda può essere rimossa in tre modi:

* usando la funzione `guides()` e specificando la scala della legenda che dovrebbe essere rimossa
* impostando l'argomento `guide` della funzione scala (`scale_xxx_xxx()`) in cui l'estetica è mappata a `FALSE`
* usare il sistema di theming specificando l'argomento `legend.position` a `"none"`

Negli esempi precedenti, il colore fornisce le stesse informazioni che ci sono nell'asse dell x, quindi la legenda non è necesssaria:
